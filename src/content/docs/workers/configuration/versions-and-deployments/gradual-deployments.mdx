...
title: Kademeli dağıtımlar
head: []
description: Kademeli dağıtımlarla Worker'larınıza kod değişikliklerini artımlı olarak dağıtın.
---

import { Example, DashButton } from "~/components";

Kademeli Dağıtımlar, trafiği sürümler arasında bölerek Worker'ların yeni [sürümlerini](/workers/configuration/versions-and-deployments/#versions) artımlı olarak dağıtma yeteneği verir.

![Kademeli Dağıtımlar](~/assets/images/workers/platform/versions-and-deployments/gradual-deployments.png)

Kademeli dağıtımları kullanarak şunları yapabilirsiniz:

- Trafiği Worker'ınızın daha yeni bir sürümüne kademeli olarak kaydırın.
- [Analitik ve log](/workers/configuration/versions-and-deployments/gradual-deployments/#observability) araçları kullanarak sürümler arasında hata oranlarını ve istisnaları izleyin.
- Yeni sürüm dağıtırken sorun fark ederseniz daha önceki kararlı sürüme [geri dönün](/workers/configuration/versions-and-deployments/rollbacks/).

## Kademeli dağıtımları kullanma

Aşağıdaki bölüm sizi kademeli dağıtımların örnek kullanımı boyunca yönlendirir. Şunları yapmak için [Wrangler](/workers/configuration/versions-and-deployments/gradual-deployments/#via-wrangler) veya Cloudflare dashboard'u kullanmayı seçeceksiniz:

- Yeni bir Worker oluşturun.
- O Worker'ın yeni sürümünü dağıtmadan yayınlayın.
- İki sürüm arasında kademeli dağıtım oluşturun.
- Yeni sürümün dağıtımını %100 trafiğe ilerletin.
